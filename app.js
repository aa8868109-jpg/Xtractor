...
// Airtable Data
// IMPORTANT: Keep secrets out of client-side source. Prefer a server-side proxy or build-time config.
let API_KEY = '';
let BASE_ID = '';
...
// 🔐 Protection System (Website Lock/Unlock)
let PROTECTION_API_KEY = '';
let PROTECTION_BASE_ID = '';
const PROTECTION_TABLE = 'Protection';
...
function getAirtableFilterFormula(fieldName, value) {
    return `({${fieldName}}='${escapeAirtableString(value)}')`;
}
...
function isProductionConfigReady() {
    return true;
}
...
async function loadSecureConfig() {
    try {
        ...
        return Boolean(config && config.ok);
    } catch (error) {
        console.warn('Unable to load secure config from server:', error);
        return false;
    }
}
...
async function checkWebsiteProtectionStatus() {
    try {
        // Avoid calling protection endpoint if server config isn't ready
        const cfgReady = await loadSecureConfig();
        if (!cfgReady) {
            console.warn('⚠️ Server config not ready — skipping protection check');
            return false;
        }
        console.log('🔐 Checking website protection status...');
        ...
        let response;
        try {
            response = await axios.get('/api/airtable/protection', { headers });
        } catch (err) {
            console.warn('⚠️ Protection endpoint failed, attempting fallback checks', err?.response?.status, err?.response?.data || err.message);
            // Try alternative capitalized path (some proxies preserve case)
            try {
                response = await axios.get('/api/airtable/Protection', { headers });
                console.log('✓ Fallback: /api/airtable/Protection succeeded');
            } catch (err2) {
                // Try config endpoint to detect if backend is configured
                try {
                    const cfg = await fetch('/api/config');
                    const cfgJson = await cfg.json();
                    console.warn('⚠️ Protection read failed; /api/config:', cfgJson);
                } catch (cfgErr) {
                    console.warn('⚠️ Failed to load /api/config', cfgErr);
                }
        ...
        if (response.data.records.length === 0) {
            console.warn('⚠️ Protection table is empty');
            return false;
        }

        // Get the first record (or the first one in the table)
        const protectionRecord = response.data.records[0];
        const fields = protectionRecord.fields;

        console.log('📊 Protection Record:', fields);

        // Read protection status
        const protectionStatus = String(fields.Select || 'Unlock').trim().toLowerCase();
        const explicitLock = protectionStatus === 'lock';
        isWebsiteLocked = explicitLock;
        lockMessage = fields.Text || 'Website is currently locked';
        lockLink = fields.Link || '';
        doctorPassword = String(fields.Password || '').trim();

        console.log(`🔐 Protection Status: ${isWebsiteLocked ? 'LOCKED' : 'UNLOCKED'}`);
        console.log(`📝 Lock Message: ${lockMessage}`);
        console.log(`🔗 Lock Link: ${lockLink}`);
        console.log(`🔑 Doctor Password: ${doctorPassword ? '***hidden***' : 'Not set'}`);
...
function showLockedWebsite() {
    ...
    if (lockedSection) {
        ...
        const safeLockLink = sanitizeLink(lockLink);
        if (safeLockLink) {
            ...
            if (linkContainer && linkEl) {
                ...
                linkEl.href = safeLockLink;
                linkEl.textContent = safeLockLink;
            }
        } else {
            const linkContainer = document.getElementById('lock-link-container');
            if (linkContainer) {
...
async function getSelectedLectureFromMode() {
    try {
        ...
        const response = await axios.get(`/api/airtable/${MODE_TABLE}`, { headers: getAirtableHeaders() });
...
async function updateStudentMode(lectureNumber, isEnabled) {
    try {
        ...
        const updateResponse = await axios.patch(
            `/api/airtable/${MODE_TABLE}/${recordId}`,
            {
                fields: {
    ...
    return new Promise((resolve) => {
        ...
        timeoutId = setTimeout(() => {
            if (!ipFound) {
                ...
                pc.close();
                // Try public IP fallback (non-private) before giving up
                fetch('https://api.ipify.org?format=json').then(r => r.json()).then(j => {
                    if (j?.ip) {
                        console.log('✓ Fallback public IP found:', j.ip);
                        deviceIP = j.ip;
                        resolve(deviceIP);
                    } else {
                        resolve(null);
...
async function checkDeviceIPConflict(studentCode, lectureNumber) {
    ...
    if (!currentIP) {
        console.warn('⚠️ لم تتمكن من الحصول على IP المحلي أو العام - سيتم السماح كحل مؤقت');
        // Mark deviceIP as unknown and allow login; IP checks are best-effort
        deviceIP = 'Unknown';
        showAlert('⚠️ لم نتمكن من تحديد عنوان الـ IP — سيتم السماح مؤقتاً', 'warning');
        return true;
    }
    ...
    try {
        ...
        const lectureResponseByIP = await axios.get(`/api/airtable/${tableName}?filterByFormula=({Device IP}='${currentIP}')`, { headers: getAirtableHeaders() });
        ...
        const lectureResponseByCode = await axios.get(`/api/airtable/${tableName}?filterByFormula=({Code}='${studentCode}')`, { headers: getAirtableHeaders() });
...
async function findStudent(studentCode) {
    ...
    try {
        const response = await axios.get(`/api/airtable/${STUDENTS_TABLE}?filterByFormula=({Code}='${studentCode}')`, { headers: getAirtableHeaders() });
...
async function saveDeviceIPForStudent(studentCode, lectureNumber) {
    try {
        ...
        const response = await axios.get(`/api/airtable/${tableName}?filterByFormula=({Code}='${studentCode}')`, { headers: getAirtableHeaders() });

        if (response.data.records.length > 0) {
            // ✅ Update existing record
            const recordId = response.data.records[0].id;
            await axios.patch(
                `/api/airtable/${tableName}/${recordId}`,
                {
                    fields: {
...
async function fetchLectureStudents(lectureNumber) {
    try {
        ...
        // Real Airtable code
        const response = await axios.get(`/api/airtable/${tableName}`, { headers: getAirtableHeaders() });
...
async function updateStudentsList() {
    ...
    if (attendedStudents.length === 0) {
        studentsList.replaceChildren(document.createElement('div'));
        const empty = document.createElement('div');
        empty.className = 'empty-list';
        empty.textContent = 'No students have scanned QR codes yet';
    ...
    attendedStudents.forEach(record => {
        const student = record.fields || {};
        const studentCode = sanitizeText(student.Code, 'N/A');
        const studentName = sanitizeText(student.Name, 'Unknown');
        const region = sanitizeText(student.Region, 'Unknown');
        ...
        item.className = `student-item ${isOutRegion ? 'out-of-region' : ''}`;
        ...
        if (isOutRegion) {
            ...
            indicator.className = 'location-alert-indicator';
            indicator.title = 'Student is out of region';
        ...
        code.textContent = `Code: ${studentCode}`;

        const status = document.createElement('div');
        status.className = 'student-status';
        status.textContent = `✓ ${qrStatus}`;
    ...
    studentsList.replaceChildren(fragment);
}
...
async function submitStudentCode() {
    const codeInput = sanitizeText(document.getElementById('student-code').value, '').replace(/[^a-zA-Z0-9-_]/g, '').slice(0, 20);
    ...
    // Doctor code check (no wait needed)
    if (doctorPassword && codeInput === doctorPassword) {
        showDoctorInterface();
    ...
    try {
        // ✅ NEW: Check protection status at login time to save API calls
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (!protectionOK) {
        console.warn('⚠️ Failed to read protection settings - allowing access');
    }
